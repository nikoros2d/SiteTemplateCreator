...
void generate_Page()
{
  std::ofstream file_Html("index.html"); // ñîçäà¸ì ôàéë ñ ðàçðåøåíèåì .html
...
void shablon_page()
{
  std::ofstream file_Html("shablon.html"); // ñîçäà¸ì ôàéë ñ ðàçðåøåíèåì .html
  file_Html << "<!DOCTYPE html>"<< endl;
  file_Html << "<html lang=\"en\">"<< endl;
  file_Html << "<head>"<< endl;
  ...
  file_Html << "<section class=\"fullscreen\">"<< endl;
  file_Html << "<div class=\"video-bg\">"<< endl;
  file_Html << "<video src=\"C:\\Users\\1\\Desktop\\222.mp4\" type=\"video/mp4\" autoplay muted loop></video>"<< endl;
  file_Html << "<div class=\"effects\"></div>"<< endl;
  file_Html << "<div class=\"video-bg__content__title\">"<< endl;
...
int get_variant(int count) {
  ...
  char s[2]; // ñòðîêà äëÿ ñ÷èòûâàíèÿ ââåä¸ííûõ äàííûõ5
  for(i=0;;){
  	cin >>("%s", s);
  	sscanf(s, "%d", &variant);
...
int main() {
  int variant,x; // âûáðàííûé ïóíêò ìåíþ
  do {
    print_menu(); // âûâîäèì ìåíþ íà ýêðàí

    variant = get_variant(3); // ïîëó÷àåì íîìåð âûáðàííîãî ïóíêòà ìåíþ
